...
public class URNTestCase {
    ...
    public void testURN() {
        ...
        {
            try {
                final String urnlit = "urn:bar:foo";
                URN urn = new URN(urnlit);
                Assert.assertEquals(urnlit, urn.toString());
            } catch (URNSyntaxException e) {
                Assert.fail();
            }
        }
    ...
    public void testURNNegativeTests() {
        // urn's must be prefixed by "urn:"
        {
            ...
            try {
                URN urn = new URN("pre:test:pre_must_be_urn");
            } catch (URNSyntaxException e) {
                caught = true;
            }
        ...
        // missing urn prefix
        {
            boolean caught = false;
            try {
                URN urn = new URN(":test:missing_urn_prefix");
            } catch (URNSyntaxException e) {
                caught = true;
            }
            Assert.assertTrue(caught);
        ...
        {
            final String urnlit = "urn,test:comma_cannot_be_first";
            boolean caught = false;
            try {
                URN urn = new URN(urnlit);
            } catch (URNSyntaxException e) {
                caught = true;
                Assert.assertEquals(urnlit, e.getInput());
        ...
        {
            final String urnlit = "urn:test,value:comma_cannot_be_first";
            boolean caught = false;
            try {
                URN urn = new URN(urnlit);
            } catch (URNSyntaxException e) {
        ...
        // urn nid too short
        {
            final String urnlit = "urn::urn_nid_too_short";
            boolean caught = false;
            try {
                URN urn = new URN(urnlit);
            } catch (URNSyntaxException e) {
        ...
        // urn nid too long
        {
            boolean caught = false;
            try {
                URN urn = new URN("urn:0123456789abcdef0123456789abcdef0:urn_nid_too_long");
            } catch (URNSyntaxException e) {
                caught = true;
            }
            Assert.assertTrue(caught);
        ...
        {
            boolean caught = false;
            final String urnlit = "urn:te%2z%25:bar";
            try {
                URN urn = new URN(urnlit);
            } catch (URNSyntaxException e) {
                caught = true;
        ...
        {
            boolean caught = false;
            final String urnlit = "urn:test:%25%0%32";
            try {
                URN urn = new URN(urnlit);
            } catch (URNSyntaxException e) {
                caught = true;
        ...
        {
            boolean caught = false;
            final String urnlit = "urn:nid:bad{syntax";
            try {
                URN urn = new URN(urnlit);
            } catch (URNSyntaxException e) {
                caught = true;
